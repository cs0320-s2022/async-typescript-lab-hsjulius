...
const sunDropdown = document.querySelector('#sun') as HTMLSelectElement
sunDropdown.addEventListener("change", postAndUpdate)
const moonDropdown = document.querySelector('#moon') as HTMLSelectElement
moonDropdown.addEventListener("change", postAndUpdate)
const risingDropdown = document.querySelector('#rising') as HTMLSelectElement
...
function postAndUpdate(): void {
  ...
  fetch("http://localhost:1212/route", {
    // Request method
    method: 'post',
...
document.addEventListener("keyup", async (event) => { if(event.key === "k" ) {
  await updateValues("Scorpio", "Taurus", "Pisces")
  postAndUpdate()
}})
